...
class PermissionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Permission
        fields = ('id', 'resource', 'action', 'resource_id', 'action_id', 'created_at')
        read_only_fields = ('id', 'created_at')
...
class AssignRoleToUserSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        if not created:
            raise serializers.ValidationError("Пользователь уже имеет эту роль.")
        return user_role
...
class AssignPermissionToRoleSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not attrs.get('permission_id') and not (attrs.get('resource_name') and attrs.get('action_name')):
            raise serializers.ValidationError(
                "Необходимо указать либо permission_id, либо resource_name и action_name."
            )
        return attrs
    ...
    def create(self, validated_data):
        ...
        if not created:
            raise serializers.ValidationError("Роль уже имеет это разрешение.")
