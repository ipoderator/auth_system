...
class MockProductViewSet(viewsets.ViewSet):
    ...
    def get_permissions(self):
        """Возвращает классы разрешений на основе действия."""
        if self.action == 'list':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='products', action='list')
            ]
        elif self.action == 'retrieve':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='products', action='read')
            ]
        elif self.action == 'create':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='products', action='create')
            ]
        elif self.action in ['update', 'partial_update']:
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='products', action='update')
            ]
        elif self.action == 'destroy':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='products', action='delete')
            ]
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            product_id = int(pk)
            product = next(
                (p for p in self.MOCK_PRODUCTS if p['id'] == product_id),
                None
            )
            if product:
...
class MockOrderViewSet(viewsets.ViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == 'list':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='orders', action='list')
            ]
        elif self.action == 'retrieve':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='orders', action='read')
            ]
        elif self.action == 'create':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='orders', action='create')
            ]
        elif self.action in ['update', 'partial_update']:
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='orders', action='update')
            ]
        elif self.action == 'destroy':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='orders', action='delete')
            ]
        return [permissions.IsAuthenticated()]
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            order_id = int(pk)
            order = next(
                (o for o in self.MOCK_ORDERS if o['id'] == order_id),
                None
            )
            if order:
...
class MockReportViewSet(viewsets.ViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == 'list':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='reports', action='list')
            ]
        elif self.action == 'retrieve':
            return [
                permissions.IsAuthenticated(),
                HasResourcePermission(resource='reports', action='read')
            ]
        return [permissions.IsAuthenticated()]
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            report_id = int(pk)
            report = next(
                (r for r in self.MOCK_REPORTS if r['id'] == report_id),
                None
            )
            if report:
